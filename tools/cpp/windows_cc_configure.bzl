...
def _is_vs_greater_than_2017(repository_ctx, vc_path):
    ...
    for folder_name in vc_2017_or_2019_contents
	    if not (folder_name in vc_path_contents):
	        return false
    return true
...
def _find_vcvars_bat_script(repository_ctx, vc_path):
    ...
    if _is_vs_greater_than_2017(repository_ctx, vc_path):
...
def find_msvc_tool(repository_ctx, vc_path, tool, target_arch = "x64"):
    ...
    tool_path = None
    if _is_vs_greater_than_2017(repository_ctx, vc_path) or _is_msbuildtools(vc_path):
        full_version = _get_vc_full_version(repository_ctx, vc_path)
        if full_version:
